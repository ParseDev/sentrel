                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {conv.subject && (
                              <p className="text-xs font-medium mt-0.5 truncate">{conv.subject}</p>
                            )}
                  ...
                  <>
                    ...
                    <div className="px-4 py-3 border-b">
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-medium text-sm">{selectedConv.contact_name || selectedConv.contact_email || selectedConv.contact_phone || "Unknown"}</h3>
                          {selectedConv.subject && <p className="text-xs text-muted-foreground mt-0.5">{selectedConv.subject}</p>}
                        </div>
                        <Badge variant="secondary" className="text-[10px]">{selectedConv.channel || "web"}</Badge>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {convMessages.map((msg) => {
                        ...
                        const emailMeta = msg.metadata as { to?: string; subject?: string; cc?: string[] }
                        return (
                          <div key={msg.id} className={`rounded-lg border p-3 ${isOutbound ? "bg-card" : "bg-muted/30"}`}>
                            <div className="flex items-center justify-between mb-1.5">
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-medium text-xs">
                                  {isOutbound ? agent.name : (selectedConv.contact_name || selectedConv.contact_email || "Contact")}
                                </span>
                                {emailMeta.to && (
                                  <span className="text-[10px] text-muted-foreground">to {isOutbound ? emailMeta.to : agent.name}</span>
                                )}
                              </div>
                            ...
                            {emailMeta.subject && (
                              <p className="text-xs font-medium mb-1.5">{emailMeta.subject}</p>
                            )}
