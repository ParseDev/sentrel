...
export function buildSchedulingMcpServer(agentId: number, orgId: number) {
  ...
  const reminderTool = tool(
    ...
    async (args) => {
      try {
        ...
        // Convert to cron: minute hour day month day-of-week
        const cron = `${dt.getUTCMinutes()} ${dt.getUTCHours()} ${dt.getUTCDate()} ${dt.getUTCMonth() + 1} *`;

        const id = await host.createScheduledTask(
          orgId, agentId,
          `[Reminder] ${args.name}`, args.instruction,
          cron, args.timezone || "UTC",
        );
        logger.info(`Reminder set: ${args.name} at ${args.datetime}`, { id });
        return {
          content: [{ type: "text", text: `Reminder set (ID: ${id}): "${args.name}" — fires at ${args.datetime} (${args.timezone || "UTC"})` }],
        };
      } catch (err) {
